...
void read(book database [], int& bookcount){
    ifstream fin("LibraryDatabase_Pickert.txt");
    if(!fin.is_open()){
            cout<<"Database does not yet contain entries.\n";
            return;
            }
    else{
        fin >> bookcount;
        ...
        while(hold){
            ...
            switch(c){
                case 'a':
                    ...
                    if(counter>0){
                        cout<<"The following book/books matched your search: \n";printbook(filler, counter);
                    }
                    else{
...
void modify(book database [], int bookcount){
    if(bookcount==0){
        ...
        if(smallcounter==0){
            ...
            cout<<smallcounter;
            if(smallcounter==0){
                cout<<"\nNo author match found. Returning to main menu.";
                return;
            }
            else{
...
void write(book database[], int number){
    ofstream fout("LibraryDatabase_Pickert.txt");
    fout<<number;
